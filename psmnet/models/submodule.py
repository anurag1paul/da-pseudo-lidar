    ...
    def forward(self, left, right, shift):
        ...
        shifted_left = F.pad(
            torch.index_select(left, 3,
                               torch.tensor([i for i in range(shift, width)]).requires_grad_().cuda()),
                             [shift, 0, 0, 0])
        shifted_right = F.pad(
            torch.index_select(right, 3,
                               torch.tensor([i for i in range(width - shift)]).requires_grad_().cuda()),
                              [shift, 0, 0, 0])
...
class feature_extraction(nn.Module):
    ...
    def forward(self, x):
        ...
        output_skip.size()[2], output_skip.size()[3]), mode='bilinear')

        output_branch2 = self.branch2(output_skip)
        output_branch2 = F.upsample(output_branch2, (
        output_skip.size()[2], output_skip.size()[3]), mode='bilinear')

        output_branch3 = self.branch3(output_skip)
        output_branch3 = F.upsample(output_branch3, (
        output_skip.size()[2], output_skip.size()[3]), mode='bilinear')

        output_branch4 = self.branch4(output_skip)
        output_branch4 = F.upsample(output_branch4, (
        output_skip.size()[2], output_skip.size()[3]), mode='bilinear')

        output_feature = torch.cat((output_raw, output_skip, output_branch4,
                                    output_branch3, output_branch2,
                                    output_branch1), 1)
